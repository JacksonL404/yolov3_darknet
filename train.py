...
def train():
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        # ----------------20240706 LWQ add opt.resume ----------------------
        # load optimizer
        if opt.resume and chkpt['optimizer'] is not None:
            optimizer.load_state_dict(chkpt['optimizer'])
            best_fitness = chkpt['best_fitness']

        # load results
        if opt.resume and chkpt.get('training_results') is not None:
            with open(results_file, 'w') as file:
                file.write(chkpt['training_results'])  # write results.txt

        start_epoch = chkpt['epoch'] + 1 if opt.resume else 0
        # ----------------20240706 LWQ add opt.resume ----------------------
        del chkpt
    ...
    dataset = LoadImagesAndLabels(train_path, img_size, batch_size,
                                  augment=True,  # True 20240706 LWQ
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/flytest.data', help='*.data path')
